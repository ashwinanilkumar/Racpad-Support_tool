...
@app.route("/api/pricing-gaps/check", methods=["POST"])
def api_pricing_gaps_check():
    """Run the 3-step pricing gap validation and return gap records."""
    from datetime import date as _date
    from pricing_validation import find_pricing_gaps

    data = request.json or {}
    check_date_str = (data.get("check_date") or "").strip()
    store_filter = (data.get("store") or "").strip()
    po_filter = (data.get("po_number") or "").strip()

    # Parse check_date (default: today)
    if check_date_str:
        try:
    ...
    # Build db_configs from saved credentials
    db_creds = auth.load_db_credentials()
    if not db_creds:
        return jsonify({"error": "DB credentials not configured. Please complete Setup."}), 400

    db_configs = {
        "racdb": {
            "host": db_creds["rac_host"],
            "port": db_creds["rac_port"],
            "dbname": db_creds["rac_dbname"],
            "user": db_creds["rac_user"],
            "password": db_creds.get("rac_password", ""),
        },
        "prcdb": {
            "host": db_creds["prc_host"],
            "port": db_creds["prc_port"],
            "dbname": db_creds["prc_dbname"],
            "user": db_creds["prc_user"],
            "password": db_creds.get("prc_password", ""),
        },
    }

    filters = {}
    if store_filter:
        filters["store"] = store_filter
    if po_filter:
        filters["po_number"] = po_filter
    ...
    try:
        ...
        return jsonify({"error": f"Pricing gap check failed: {e}",
                        "traceback": traceback.format_exc()}), 500
...
@app.route("/api/pricing-gaps/check-stream", methods=["POST"])
def api_pricing_gaps_check_stream():
    """SSE streaming endpoint — sends step-by-step progress then final result."""
    import queue
    from datetime import date as _date
    from pricing_validation import find_pricing_gaps
    ...
    if not db_creds:
        def _err():
            yield f"data: {json.dumps({'type':'error','message':'DB credentials not configured. Please complete Setup.'})}\n\n"
        return Response(_err(), mimetype="text/event-stream")
    ...
    # Use a thread-safe queue to collect progress events from the worker thread
    progress_q = queue.Queue()

    def _on_progress(step, status, detail):
        progress_q.put({"type": "progress", "step": step, "status": status, "detail": detail})

    def _generate():
        import threading

        result_holder = [None, None]  # [gaps, error]
